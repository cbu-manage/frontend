...
interface InputBoxProps extends React.InputHTMLAttributes<HTMLInputElement> {
  ...
  insetLabel?: string;
  /** outline: 흰 배경 + 회색 테두리 + rounded-xl (신청서 전용) */
...
export default function InputBox({
  ...
  return (
    <div className="w-full space-y-1.5">
      {label && (
        <label className="block text-body-sm font-medium text-gray-900">
          {label}
      ...
      <div className="relative">
        <input
          ...
          disabled={disabled}
          className={`
            w-full p-4 text-base font-medium tracking-[-0.048px] leading-normal outline-none transition-all duration-150 border
            ${isOutline ? "rounded-xl" : "rounded-lg"}
            ${
              disabled
                ? "bg-gray-100 border-gray-200 text-gray-500 cursor-not-allowed placeholder:text-gray-400"
                : hasError
                  ? "bg-gray-0 border-notice text-gray-600 placeholder:text-gray-600"
                  : isOutline
                    ? "bg-gray-0 border-gray-200 text-gray-900 placeholder:text-gray-600 focus:border-brand focus:ring-1 focus:ring-brand"
