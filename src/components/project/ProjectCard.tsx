...
export function ProjectRow({
  ...
  return (
    <Link
      href={`/project/${id}`}
      className="border-b border-gray-100 py-4 hover:bg-gray-50 transition-colors cursor-pointer w-full block"
    >
      {/* ========== 모바일 카드형 (sm 미만) ========== */}
      ...
      <div className="hidden sm:grid sm:grid-cols-6 md:grid-cols-10 lg:grid-cols-13 text-center items-center">
        ...
        <div className="hidden md:block md:col-span-2 text-gray-500 text-sm">
          {author}
        </div>
        <div className="hidden lg:block lg:col-span-2 text-gray-400 text-sm">
          {time}
        </div>
      </div>
    </Link>
