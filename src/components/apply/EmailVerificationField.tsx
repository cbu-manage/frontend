...
export default function EmailVerificationField({
  ...
  const { sendEmailToServer, verifyCodeWithServer, isVerificationSent, isSending, isVerifying } =
    useVerifyEmail();
  ...
  const handleButtonClick = async () => {
    ...
    if (!isVerificationSent) {
      ...
      const { success, responseMessage } = await verifyCodeWithServer(email, verificationCode);
      if (success) {
        onVerify();
      } else {
  ...
  return (
    <div className="space-y-1.5">
      <p id="email-verification-label" className="text-body-sm font-medium text-gray-900">
        이메일 입력 <span className="text-notice">*</span>
      </p>
      <div className="flex gap-2">
        {/* 이메일 입력 + 고정 도메인 */}
        <div
          className={`flex flex-1 min-w-0 items-center rounded-lg border transition-all duration-150 ${
        ...
        >
          <input
            ...
            aria-describedby={displayError ? "email-verification-error" : undefined}
        ...
        <input
          ...
          aria-label="인증번호 입력"
          aria-describedby={displayError ? "email-verification-error" : undefined}
          className="w-32 shrink-0 rounded-lg px-4 py-4 text-base font-medium tracking-[-0.048px] leading-normal border border-transparent bg-gray-50 text-gray-900 placeholder:text-gray-600 outline-none transition-all duration-150 focus:bg-gray-0 focus:border-brand focus:ring-1 focus:ring-brand disabled:bg-gray-100 disabled:text-gray-500 disabled:cursor-not-allowed"
        />
        ...
        <button
          ...
          className={`shrink-0 rounded-lg px-5 py-4 text-base font-medium transition-colors duration-200 ${
            isVerified
      ...
      {displayError && (
        <p id="email-verification-error" aria-live="polite" className="text-xs text-notice flex items-center gap-1 mt-1">
